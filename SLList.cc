...
public:
    ...
    void push_front(const T &d)
    {
        ...
        if (size_ == 0)
        { // Caso especial en el que la lista esta vacia, n es tanto first_ como last_.
            last_ = n;
        }
    ...
    void push_back(const T &d)
    {
        ...
        if (empty())
        { // Si la lista está vacía (first_ es nullptr o size_ es 0), el nuevo nodo será el único elemento.
            first_ = n;
    ...
    void insert(const T &d, unsigned int pos)
    {
        ...
        if (pos > size_)
            return; // Si la posición es inválida (mayor que `size_`), no se inserta nada.
    ...
    void reverse_list()
    {
        ...
        last_ = first_; // El primer nodo pasará a ser el último después de la inversión.

        while (current != nullptr)
        {
            next = current->getNext(); // Guardamos el siguiente nodo.
            current->setNext(prev);    // Invertimos el puntero del nodo actual.
            prev = current;            // Movemos prev al nodo actual.
            current = next;            // Avanzamos al siguiente nodo.
        }

        first_ = prev; // Actualizamos el puntero `first_` al nuevo primer nodo.
    }
};
